#!/usr/bin/env bash
# Boot the Linux Oracle capsule with virtio-blk-pci and capture live JSONL.
...
SERIAL_LOG="$OUT_DIR/serial_blk.log"
JSONL_OUT="${1:-$OUT_DIR/virtio_blk_oracle_events.jsonl}"
PROMOTE="${PROMOTE_CAPTURE:-1}"
QEMU_BIN="${QEMU_SYSTEM_X86_64:-qemu-system-x86_64}"
DISK_IMG="$OUT_DIR/oracle_blk.img"
CAPTURE_SOURCE="$ROOT_DIR/tools/trace/virtio_blk_oracle_capture.c"
...
if ! command -v "$QEMU_BIN" >/dev/null 2>&1; then
  echo "RAMEN_CAPTURE_VIRTIO_BLK_ORACLE: fail error=missing_qemu" >&2
  exit 1
fi

if ! KERNEL_IMAGE="$(resolve_kernel)"; then
  echo "RAMEN_CAPTURE_VIRTIO_BLK_ORACLE: fail error=missing_kernel" >&2
...
if ! grep -q 'RAMEN_VIRTIO_BLK_CAPTURE: ok' "$SERIAL_LOG"; then
  echo "RAMEN_CAPTURE_VIRTIO_BLK_ORACLE: fail error=capture_guest_failed" >&2
...
begin = "RAMEN_VIRTIO_BLK_CAPTURE_BEGIN\n"
end = "\nRAMEN_VIRTIO_BLK_CAPTURE_END"
...
echo "RAMEN_CAPTURE_VIRTIO_BLK_ORACLE: ok jsonl=$JSONL_OUT"

if [[ "$PROMOTE" == "1" ]]; then
  bash "$ROOT_DIR/tools/trace/promote_virtio_blk_capture.sh" "$JSONL_OUT"
fi