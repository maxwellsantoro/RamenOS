#!/usr/bin/env bash
# Boot the Linux Oracle capsule with virtio-blk-pci and capture harness sector JSONL.
...
SERIAL_LOG="$OUT_DIR/serial_blk_sector.log"
JSONL_OUT="${1:-$OUT_DIR/virtio_blk_sector_oracle_events.jsonl}"
PROMOTE="${PROMOTE_CAPTURE:-1}"
QEMU_BIN="${QEMU_SYSTEM_X86_64:-qemu-system-x86_64}"
DISK_IMG="$OUT_DIR/oracle_blk.img"
CAPTURE_SOURCE="$ROOT_DIR/tools/trace/virtio_blk_sector_oracle_capture.c"
...
if ! command -v "$QEMU_BIN" >/dev/null 2>&1; then
  echo "RAMEN_CAPTURE_VIRTIO_BLK_SECTOR_ORACLE: fail error=missing_qemu" >&2
  exit 1
fi

if ! KERNEL_IMAGE="$(resolve_kernel)"; then
  echo "RAMEN_CAPTURE_VIRTIO_BLK_SECTOR_ORACLE: fail error=missing_kernel" >&2
  echo "Set S2_COMPAT_KERNEL or S2_COMPAT_KERNEL_URL" >&2
  exit 1
fi

if [[ ! -f "$DISK_IMG" ]]; then
  dd if=/dev/zero of="$DISK_IMG" bs=1M count=4 status=none 2>/dev/null \
...
"$QEMU_BIN" \
  ...
  -drive "file=$DISK_IMG,if=none,format=raw,id=blkdrv" \
  -device virtio-blk-pci,drive=blkdrv,bus=pcie.0,addr=0x4,disable-modern=on,disable-legacy=off \
  -no-reboot \
  -nographic \
  -serial "file:$SERIAL_LOG"

if ! grep -q 'RAMEN_VIRTIO_BLK_SECTOR_CAPTURE: ok' "$SERIAL_LOG"; then
  echo "RAMEN_CAPTURE_VIRTIO_BLK_SECTOR_ORACLE: fail error=capture_guest_failed" >&2
...
begin = "RAMEN_VIRTIO_BLK_SECTOR_CAPTURE_BEGIN\n"
end = "\nRAMEN_VIRTIO_BLK_SECTOR_CAPTURE_END"
...
echo "RAMEN_CAPTURE_VIRTIO_BLK_SECTOR_ORACLE: ok jsonl=$JSONL_OUT"

if [[ "$PROMOTE" == "1" ]]; then
  bash "$ROOT_DIR/tools/trace/promote_virtio_blk_sector_capture.sh" "$JSONL_OUT"
fi