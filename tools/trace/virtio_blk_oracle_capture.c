/*
 * Linux Oracle capsule init for S13 virtio-blk PCI/MMIO capture.
 *
 * Performs legacy virtio-blk-pci bring-up (features, queue, capacity) against
 * live QEMU hardware and emits driver_protocol_trace_v0-compatible JSONL on ttyS0.
 */
...
static void record_capacity(void) {
    (void)record_mmio_read32(VIRTIO_PCI_CONFIG_OFF + 0u);
    (void)record_mmio_read32(VIRTIO_PCI_CONFIG_OFF + 4u);
}
...
int main(void) {
    ...
    write_str("RAMEN_VIRTIO_BLK_CAPTURE: boot\n");
    ...
    if (wait_for_path(PCI_SYSFS "/vendor") != 0) {
        write_str("RAMEN_VIRTIO_BLK_CAPTURE: fail pci_device_missing\n");
        poweroff();
    }

    write_line("RAMEN_VIRTIO_BLK_CAPTURE_BEGIN");
    write_line("{\"metadata\":{\"oracle\":\"linux-virtio-blk\",\"device_model\":\"virtio-blk-pci\","
               "\"pci_vendor_id\":6900,\"pci_device_id\":4097,"
               "\"pci_bdf\":\"0000:00:04.0\",\"capture_tool\":\"virtio_blk_oracle_capture\"}}");

    if (read_pci_config_u16(0, &vendor) != 0) {
        write_str("RAMEN_VIRTIO_BLK_CAPTURE: fail vendor_read\n");
        poweroff();
    }
    emit_event("pci_config_read", 255, 0, 2, vendor);

    if (read_pci_config_u16(2, &device) != 0) {
        write_str("RAMEN_VIRTIO_BLK_CAPTURE: fail device_read\n");
        poweroff();
    }
    emit_event("pci_config_read", 255, 2, 2, device);

    if (map_bar0() != 0) {
        write_str("RAMEN_VIRTIO_BLK_CAPTURE: fail bar0_map\n");
        poweroff();
    }
    ...
    if (host_features != 0) {
        ...
        if ((status & VIRTIO_CONFIG_S_FEATURES_OK) == 0) {
            write_str("RAMEN_VIRTIO_BLK_CAPTURE: fail features_ok_rejected\n");
            poweroff();
        }
    }

    record_capacity();
    setup_queue(0, QUEUE_PFN, DEFAULT_QUEUE_SIZE);
    ...
    write_line("RAMEN_VIRTIO_BLK_CAPTURE_END");
    write_str("RAMEN_VIRTIO_BLK_CAPTURE: ok\n");
    poweroff();
    return 0;
}