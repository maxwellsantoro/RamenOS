#!/usr/bin/env bash
# Boot the Linux Oracle capsule with virtio-net-pci and capture live JSONL.
...
JSONL_OUT="${1:-$OUT_DIR/virtio_net_oracle_events.jsonl}"
...
if ! command -v "$QEMU_BIN" >/dev/null 2>&1; then
  echo "RAMEN_CAPTURE_VIRTIO_NET_ORACLE: fail error=missing_qemu" >&2
  exit 1
fi

if ! KERNEL_IMAGE="$(resolve_kernel)"; then
  echo "RAMEN_CAPTURE_VIRTIO_NET_ORACLE: fail error=missing_kernel" >&2
  echo "Set S2_COMPAT_KERNEL or S2_COMPAT_KERNEL_URL" >&2
  exit 1
fi

bash "$ROOT_DIR/tools/trace/build_oracle_capture_initrd.sh"
...
if ! grep -q 'RAMEN_VIRTIO_NET_CAPTURE: ok' "$SERIAL_LOG"; then
  echo "RAMEN_CAPTURE_VIRTIO_NET_ORACLE: fail error=capture_guest_failed" >&2
...
begin = "RAMEN_VIRTIO_NET_CAPTURE_BEGIN\n"
end = "\nRAMEN_VIRTIO_NET_CAPTURE_END"
...
echo "RAMEN_CAPTURE_VIRTIO_NET_ORACLE: ok jsonl=$JSONL_OUT"

if [[ "$PROMOTE" == "1" ]]; then
  bash "$ROOT_DIR/tools/trace/promote_virtio_net_capture.sh" "$JSONL_OUT"
fi