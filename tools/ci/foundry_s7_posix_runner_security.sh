...
# Test 4: Sandbox enabled by default
echo "Test 4: Verifying sandbox is enabled by default..."
...
grep -q "Sandbox: ENABLED" "$EVIDENCE_DIR/test4_sandbox_default.log" || {
    echo "FAIL: Expected sandbox enabled marker not found"
    cat "$EVIDENCE_DIR/test4_sandbox_default.log"
    exit 1
}
...
echo "PASS: Sandbox is enabled by default"
...
# Test 8: Full sandbox enforcement
echo "Test 8: Verifying all sandbox controls work together..."
cargo test -p runtime_supervisor --features posix_runner_v0_dev sandbox_full_enforcement -- --nocapture \
    ...
    echo "FAIL: Full sandbox test failed"
    cat "$EVIDENCE_DIR/test8_full_sandbox.log"
    exit 1
}

grep -q "test result: ok" "$EVIDENCE_DIR/test8_full_sandbox.log" || {
    echo "FAIL: Full sandbox test did not pass"
    cat "$EVIDENCE_DIR/test8_full_sandbox.log"
    exit 1
}

echo "PASS: Full sandbox enforcement verified"
...
- **Expected Output**: "Sandbox: ENABLED" and no "WARNING: SANDBOX DISABLED"
...
- **Expected Output**: Integration test verifies all sandbox controls work together
...
4. ✅ Sandbox defaults to enabled
5. ✅ Seccomp filter actually blocks dangerous syscalls (execve, socket, etc.)
6. ✅ Resource limits are actually enforced (RLIMIT_NPROC prevents fork)
7. ✅ Chroot actually confines filesystem access
8. ✅ All sandbox controls work together

## Evidence Discipline

- Assertions are backed by executable integration tests in runtime_supervisor/src/sandbox.rs
- Integration tests verify actual security enforcement, not just log messages
- Gate does not rely on external store service availability
- PASS/FAIL is deterministic and auditable via captured logs
- Security controls are tested in isolation and together
EOF
