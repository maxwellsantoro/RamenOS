//! virtio-net initialization driver distilled from the Oracle trace.
//!
//! Replays legacy PCI discovery, feature negotiation, queue setup, and MAC
...
pub const VIRTIO_NET_DEVICE_ID: u64 = 0x1000;
...
pub const NET_CONFIG_OFFSET: u64 = 0x14;
...
pub const RX_QUEUE_INDEX: u64 = 0;
pub const TX_QUEUE_INDEX: u64 = 1;
pub const RX_QUEUE_PFN: u64 = 0x100000;
pub const TX_QUEUE_PFN: u64 = 0x101000;
pub const DEFAULT_QUEUE_SIZE: u64 = 256;

/// Run the virtio-net init sequence recorded in the Reference Vault oracle trace.
pub fn init_virtio_net(device: &mut MockPciDevice<'_>) -> ReplayResult<()> {
    discover_pci(device)?;
    let host_features = negotiate_features(device)?;
    observe_mac(device)?;
    setup_queue(device, RX_QUEUE_INDEX, RX_QUEUE_PFN, DEFAULT_QUEUE_SIZE)?;
    setup_queue(device, TX_QUEUE_INDEX, TX_QUEUE_PFN, DEFAULT_QUEUE_SIZE)?;
...
fn observe_mac(device: &mut MockPciDevice<'_>) -> ReplayResult<()> {
    for word in 0..3 {
        let offset = NET_CONFIG_OFFSET + (word * 2);
        device.mmio_read(0, offset, 2)?;
    }
    Ok(())
}
...
pub fn replay_vault_init_trace() -> Result<(), crate::DriverReplayError> {
    ...
    let mut device = MockPciDevice::new(&events);
    init_virtio_net(&mut device).map_err(|err| crate::DriverReplayError::Replay(format!("{err:?}")))
}
...
mod tests {
    ...
    fn replay_init_from_events_rejects_incomplete_trace() {
        let trace = [
            ...
            PciReplayEvent::pci_config_read(0x02, 2, VIRTIO_NET_DEVICE_ID),
